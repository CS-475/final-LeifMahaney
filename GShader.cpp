...
public:
    ...
    void shadeRow(int x, int y, int count, GPixel row[]) override {
        ...
        for (int i = 0; i < count; ++i) {
            dstP = {x + i + 0.5f, y + 0.5f};
            fInverse->mapPoints(&srcP, &dstP, 1);
            ...
            int clampX = std::clamp(static_cast<int>(srcX), 0, fBitmap.width() - 1);
            int clampY = std::clamp(static_cast<int>(srcY), 0, fBitmap.height() - 1);

            row[i] = *fBitmap.getAddr(clampX, clampY);
        }
    }
    ...
    float applyTileMode(float coord, int limit, GTileMode mode) const {
        switch (mode) {
            ...
            case GTileMode::kMirror:
                int wholeP = static_cast<int>(std::floor(coord / limit));
                float fracP = coord - wholeP * limit;
                return (wholeP % 2 == 0) ? fracP : limit - 1 - fracP;
        }
        return coord; 
    }
};
...
public:
    ...
    void shadeRow(int x, int y, int count, GPixel row[]) override {
        for (int i = 0; i < count; ++i) {
            GPoint deviceP = {x + i + 0.5f, y + 0.5f};
            GPoint localP;
            fLocalMatrix.mapPoints(&localP, &deviceP, 1);

            float t = applyTileMode(localP.x, fTileMode);
